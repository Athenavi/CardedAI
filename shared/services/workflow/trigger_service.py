...
class TriggerService:
    ...
    async def _cron_callback(
        ...
        try:
            ...
            if graph_data is None:
                ...
                if graph_data is None:
                    logger.logger(f"f"[TriggerService] 无法加载工作流 {workflow_id} 的图结构")
                    return
            ...
            logger.logger(f"f"[TriggerService] Cron 触发执行失败: {exc}")
        ...
        async def _event_handler(event_data: Dict = None):
            ...
            try:
                ...
                if wf_graph is None:
                    ...
                    if wf_graph is None:
                        logger.logger(f"f"[TriggerService] 无法加载工作流 {workflow_id}")
                        return
                ...
                logger.logger(f"f"[TriggerService] 事件触发执行失败: {exc}")
    ...
    async def _load_graph_from_db(self, workflow_id: int) -> Optional[Any]:
        ...
        try:
            ...
            logger.logger(f"f"[TriggerService] 加载工作流 {workflow_id} 失败: {exc}")
            return None
