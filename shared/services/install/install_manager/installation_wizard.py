...
class InstallationWizardService:
    ...
    def _update_env_file(self, updates: Dict[str, str]):
        ...
        try:
            ...
            if self.config_file.exists():
                ...
                logger.logger(f"f"[ERROR] 请检查以下可能的问题:")
    ...
    def import_sample_data(
        ...
        try:
            ...
            try:
                ...
                def run_in_thread():
                    ...
                    try:
                        ...
                        logger.logger(f"f"导入示例数据时事件循环异常: {e}")
                        raise
                    finally:
                        new_loop.close()
                        # 线程安全地恢复单例
            ...
            import traceback
            logger.logger(f"f"导入示例数据失败: {str(e)}")
    ...
    def create_admin_user(
        ...
        try:
            ...
            import traceback
            logger.logger(f"f"创建管理员账号失败: {str(e)}")
            logger.logger(f"traceback.format_exc())
            return {
                'success': False,
        ...
        try:
            ...
            if install_info.get('import_sample_data', False):
                ...
                if sample_result['success']:
                    ...
                    logger.logger(f"f"✗ 示例数据导入失败: {sample_result.get('error', '未知错误')}")
