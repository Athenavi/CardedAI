...
class DatabaseMigrationService:
    ...
    async def run_migration(self) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        try:
            ...
            logger.logger(f"f"[Migration] 工作目录: {self.project_root}")
            ...
            # 输出 stdout
            if result.stdout:
                logger.logger(f"f"[Migration STDOUT]\n{result.stdout}")
                for line in result.stdout.split('\n'):
                    if line.strip():
                        yield {
                            'type': 'info',
            ...
            # 输出 stderr
            if result.stderr:
                logger.logger(f"f"[Migration STDERR]\n{result.stderr}")
            ...
            logger.logger(f"f"[Migration] 退出码: {result.returncode}")
            ...
            logger.logger(f"f"[Migration ERROR] {error_msg}")
            yield {
                'type': 'error',
                'message': error_msg,
                'timestamp': time.time()
