...
class DatabaseMigrationService:
    ...
    async def run_migration(self) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        try:
            ...
            logger(f"[Migration] 工作目录: {self.project_root}")
            ...
            logger(f"[Migration] 退出码: {result.returncode}")
            ...
            logger(f"[Migration ERROR] {error_msg}")
            yield {
                'type': 'error',
                'message': error_msg,
                'timestamp': time.time()
