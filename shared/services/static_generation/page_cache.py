...
class PageCacheService:
    ...
    async def get_page(self, url: str, user_role: str = "anonymous",
        ...
        if cached_content:
            logger.logger(f"[PageCache] HIT: {url} (role: {user_role})")
            return cached_content

        logger.logger(f"[PageCache] MISS: {url} (role: {user_role})")
        return None
    ...
    async def set_page(self, url: str, content: str, user_role: str = "anonymous",
        ...
        logger.logger(f"[PageCache] SET: {url} (role: {user_role}, ttl: {ttl}s)")
    ...
    async def invalidate_page(self, url: str, user_role: str = "anonymous",
        ...
        logger.logger(f"[PageCache] INVALIDATE: {url} (role: {user_role})")
    ...
    async def invalidate_pattern(self, pattern: str) -> int:
        ...
        logger.logger(f"[PageCache] INVALIDATE PATTERN: {pattern}")
        return 0
