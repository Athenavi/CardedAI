...
class TranslationService:
        ...
        for lang_code in self.supported_languages.keys():
            ...
            if os.path.exists(translation_file):
                try:
                    ...
                    logger.logger(f"f"Failed to load translations for {lang_code}: {e}")
    ...
    def _save_translations(self, language: str):
        ...
        try:
            ...
            logger.logger(f"f"Failed to save translations for {language}: {e}")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=params) as response:
                    if response.status == 200:
                        ...
                        logger.logger(f"f"Translation API error: {response.status}")
                        return text

        except Exception as e:
            logger.logger(f"f"Auto translation failed: {e}")
            return text
