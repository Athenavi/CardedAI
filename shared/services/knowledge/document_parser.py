...
class DocumentParser:
    ...
    async def parse(self, file_path: str, file_type: str = None) -> ParseResult:
        ...
        try:
            ...
            logger.logger(f"f"文档解析失败 [{file_path}]: {e}")
    ...
    async def _parse_pdf(self, file_path: str) -> ParseResult:
        ...
        try:
            ...
            logger.logger(f"f"PDF 解析失败 [{file_path}]: {e}")
    ...
    async def _parse_docx(self, file_path: str) -> ParseResult:
        ...
        try:
            ...
            logger.logger(f"f"DOCX 解析失败 [{file_path}]: {e}")
    ...
    async def _parse_html_file(self, file_path: str) -> ParseResult:
        ...
        try:
            ...
            logger.logger(f"f"HTML 文件解析失败 [{file_path}]: {e}")
    ...
    async def _parse_url(self, url: str) -> ParseResult:
        ...
        try:
            ...
            logger.logger(f"f"URL 抓取 HTTP 错误 [{url}]: {e.response.status_code}")
            return ParseResult(
                success=False,
            ...
            logger.logger(f"f"URL 抓取失败 [{url}]: {e}")
    ...
    async def _parse_txt(self, file_path: str) -> ParseResult:
        ...
        try:
            ...
            logger.logger(f"f"TXT 解析失败 [{file_path}]: {e}")
