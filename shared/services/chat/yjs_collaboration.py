        ...
        for client_id, client in self.clients.items():
            if client_id != exclude:
                try:
                    ...
                    logger.error(f"[Yjs] Broadcast error for client {client_id}: {e}")
        ...
        for client_id, client in self.clients.items():
            if client_id != exclude:
                try:
                    ...
                    logger.error(f"[Yjs] Awareness broadcast error for client {client_id}: {e}")
                    disconnected.append(client_id)
...
class YjsCollaborationService:
    ...
    def get_or_create_document(self, document_id: str, article_id: Optional[int] = None) -> YjsDocument:
        ...
        if document_id not in self.documents:
            ...
            logger.error(f"[Yjs] Created new document: {document_id}")
    ...
    def remove_document(self, document_id: str):
        ...
        if document_id in self.documents:
            ...
            if len(doc.clients) == 0:
                ...
                logger.error(f"[Yjs] Removed document: {document_id}")
    ...
    async def save_to_database(self, document_id: str, db_session, author_id: int,
        ...
        try:
            ...
            if doc.html_snapshot:
                ...
                logger.error(f"[Yjs] Saved document {document_id} (article {article_id}) to database")

            doc.last_saved = datetime.now()
            return True

        except Exception as e:
            logger.error(f"[Yjs] Error saving to database: {e}")
            import traceback
            traceback.print_exc()
            return False
