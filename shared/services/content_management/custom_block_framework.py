...
class BlockPluginManager:
    ...
    def load_plugin(self, plugin_instance: CustomBlockPlugin) -> bool:
        ...
        if plugin_name in self.plugins:
            logger.logger(f"⚠️ 插件 '{plugin_name}' 已加载")
            return False
        ...
        try:
            ...
            logger.logger(f"✅ 插件 '{plugin_name}' v{plugin_instance.version} 加载成功")
            logger.logger(f"   注册了 {len(plugin_info.blocks)} 个块类型")

            return True

        except Exception as e:
            logger.logger(f"❌ 插件 '{plugin_name}' 加载失败: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def unload_plugin(self, plugin_name: str) -> bool:
        ...
        if plugin_name not in self.plugins:
            logger.logger(f"⚠️ 插件 '{plugin_name}' 未加载")
            return False
        ...
        try:
            ...
            logger.logger(f"✅ 插件 '{plugin_name}' 已卸载")
            return True

        except Exception as e:
            logger.logger(f"❌ 插件 '{plugin_name}' 卸载失败: {e}")
            return False
    ...
    def auto_discover_plugins(self) -> int:
        ...
        if not self.plugin_dir.exists():
            logger.logger(f"📁 插件目录不存在: {self.plugin_dir}")
            return 0
        ...
        for plugin_path in self.plugin_dir.glob("*/plugin.py"):
            try:
                ...
                logger.logger(f"⚠️ 加载插件 {plugin_path} 失败: {e}")

        logger.logger(f"🔍 自动发现完成，成功加载 {loaded_count} 个插件")
        return loaded_count
