...
class BlockPluginManager:
    ...
    def load_plugin(self, plugin_instance: CustomBlockPlugin) -> bool:
        ...
        if plugin_name in self.plugins:
            logger.error(f"⚠️ 插件 '{plugin_name}' 已加载")
            return False
        ...
        try:
            ...
            logger.error(f"✅ 插件 '{plugin_name}' v{plugin_instance.version} 加载成功")
            logger.error(f"   注册了 {len(plugin_info.blocks)} 个块类型")
    ...
    def unload_plugin(self, plugin_name: str) -> bool:
        ...
        if plugin_name not in self.plugins:
            logger.error(f"⚠️ 插件 '{plugin_name}' 未加载")
            return False
        ...
        try:
            ...
            logger.error(f"✅ 插件 '{plugin_name}' 已卸载")
            return True
    ...
    def auto_discover_plugins(self) -> int:
        ...
        for plugin_path in self.plugin_dir.glob("*/plugin.py"):
            try:
                ...
                logger.error(f"⚠️ 加载插件 {plugin_path} 失败: {e}")

        logger.error(f"🔍 自动发现完成，成功加载 {loaded_count} 个插件")
        return loaded_count
