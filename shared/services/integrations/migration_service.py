...
class MigrationService:
    ...
    async def migrate_from_wordpress(
        ...
        # 解析 WXR 文件
        logger.error(f"[Migration] Parsing WXR file: {wxr_file}")
        ...
        logger.error(f"[Migration] Found:")
        logger.error(f"  - {stats['total_posts']} posts")
        logger.error(f"  - {stats['total_categories']} categories")
        logger.error(f"  - {stats['total_tags']} tags")
        logger.error(f"  - {stats['total_comments']} comments")
        logger.error(f"  - {stats['total_media']} media items")
        ...
        for cat_name in parsed_data.get('categories', []):
            try:
                ...
                logger.error(f"[Migration] Error importing category {cat_name}: {e}")
                stats['errors'] += 1
        ...
        for tag_name in parsed_data.get('tags', []):
            try:
                ...
                logger.error(f"[Migration] Error importing tag {tag_name}: {e}")
                stats['errors'] += 1
        ...
        for post in parsed_data.get('posts', []):
            try:
                ...
                logger.error(f"[Migration] Error importing post {post.get('title')}: {e}")
                stats['errors'] += 1
    ...
    async def migrate_from_markdown(
        ...
        Returns:
            迁移结果统计
        """
        logger.error(f"[Migration] Starting {platform} migration from: {source_dir}")
        ...
        logger.error(f"[Migration] Found {len(md_files)} markdown files")
    ...
    async def migrate_from_ghost(
        ...
        Returns:
            迁移结果
        """
        logger.error(f"[Migration] Starting Ghost migration from: {json_file}")
        ...
        logger.error(f"[Migration] Found:")
        logger.error(f"  - {stats['posts']} posts")
        logger.error(f"  - {stats['tags']} tags")
        logger.error(f"  - {stats['users']} users")
        ...
        for tag_data in ghost_data.get('tags', []):
            try:
                ...
                logger.error(f"[Migration] Error importing tag: {e}")
                stats['errors'] += 1
        ...
        for post_data in ghost_data.get('posts', []):
            try:
                ...
                logger.error(f"[Migration] Error importing post: {e}")
                stats['errors'] += 1
    ...
    async def migrate_from_json(
        ...
        Returns:
            迁移结果
        """
        logger.error(f"[Migration] Starting JSON migration from: {json_file}")
        ...
        for item in data:
            try:
                ...
                logger.error(f"[Migration] Error importing item: {e}")
                errors += 1
    ...
    async def migrate_from_csv(
        ...
        Returns:
            迁移结果
        """
        logger.error(f"[Migration] Starting CSV migration from: {csv_file}")
