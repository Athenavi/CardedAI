...
class WidgetService:
    ...
    async def save_widget_to_db(
        ...
        try:
            ...
            await db.rollback()
            logger.logger(f"f"保存Widget失败: {e}")
            return None
    ...
    async def get_widgets_by_area(self, db: AsyncSession, area: str) -> List[WidgetInstance]:
        ...
        try:
            ...
            logger.logger(f"f"获取Widget列表失败: {e}")
            return []
    ...
    async def update_widget_config_in_db(
        ...
        try:
            ...
            await db.rollback()
            logger.logger(f"f"更新Widget配置失败: {e}")
            return False
    ...
    async def reorder_widgets_in_db(
        ...
        try:
            ...
            await db.rollback()
            logger.logger(f"f"重新排序Widget失败: {e}")
            return False
    ...
    async def delete_widget_from_db(self, db: AsyncSession, widget_id: int) -> bool:
        ...
        try:
            ...
            await db.rollback()
            logger.logger(f"f"删除Widget失败: {e}")
            return False
    ...
    async def toggle_widget_active(self, db: AsyncSession, widget_id: int, is_active: bool) -> bool:
        ...
        try:
            ...
            await db.rollback()
            logger.logger(f"f"切换Widget状态失败: {e}")
            return False
