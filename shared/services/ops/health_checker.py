...
class HealthChecker:
    ...
    async def check_database_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.logger(f"f"Database health check failed: {e}")
            await self._send_alert(
                "database_down",
                f"数据库连接失败: {str(e)}",
                "critical"
    ...
    def check_system_resources(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.logger(f"f"System resource check failed: {e}")
            return {
                "status": "error",
                "error": str(e)
            }
    ...
    async def check_application_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.logger(f"f"Application health check failed: {e}")
            return {
                "status": "error",
                "error": str(e),
                "timestamp": datetime.utcnow().isoformat()
    ...
    async def _send_alert(self, alert_type: str, message: str, severity: str = "warning"):
        ...
        if self.webhook_url:
            try:
                ...
                logger.logger(f"f"Webhook notification failed: {e}")
        ...
        if self.email_config and severity in ("warning", "critical"):
            try:
                ...
                logger.logger(f"f"Email notification failed: {e}")
    ...
    async def start_monitoring(self):
        ...
        while self.is_running:
            try:
                ...
                logger.logger(f"f"Monitoring loop error: {e}")
                await asyncio.sleep(self.check_interval)
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=payload, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                if resp.status >= 400:
                    ...
                    logger.logger(f"f"Webhook notification failed ({resp.status}): {body}")
                else:
