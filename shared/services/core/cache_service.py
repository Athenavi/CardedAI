    ...
    def __init__(self, use_redis: bool = False, redis_config: Dict[str, Any] = None,
        ...
        if use_redis and REDIS_AVAILABLE:
            try:
                ...
                logger.logger(f"[CacheService] Redis连接失败: {e}, 使用内存缓存")
    ...
    def get(self, key: str) -> Optional[Any]:
        ...
        if self.use_redis and self.redis_client:
            try:
                ...
                logger.logger(f"[CacheService] Redis获取失败: {e}, 降级到内存缓存")
                self.use_redis = False
    ...
    def set(self, key: str, value: Any, ttl: int = None):
        ...
        if self.use_redis and self.redis_client:
            try:
                ...
                logger.logger(f"[CacheService] Redis存储失败: {e}, 降级到内存缓存")
                self.use_redis = False
    ...
    def delete(self, key: str):
        ...
        if self.use_redis and self.redis_client:
            try:
                ...
                logger.logger(f"[CacheService] Redis删除失败: {e}")
    ...
    def clear(self):
        ...
        if self.use_redis and self.redis_client:
            try:
                ...
                logger.logger(f"[CacheService] Redis清空失败: {e}")
...
class PageCacheService:
    ...
    def get_page(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.logger(f"[PageCache] 读取缓存失败: {e}")
            return None
    ...
    def set_page(self, url: str, content: str, status_code: int = 200,
        ...
        try:
            ...
            logger.logger(f"[PageCache] 保存缓存失败: {e}")
    ...
    def invalidate_page(self, url: str):
        ...
        try:
            ...
            logger.logger(f"[PageCache] 删除缓存失败: {e}")
    ...
    def clear_all(self):
        ...
        try:
            ...
            logger.logger(f"[PageCache] 清空缓存失败: {e}")
        ...
        for item in self.warmup_queue:
            ...
            try:
                ...
                if content:
                    ...
                    warmed_count += 1
                    logger.logger(f"[CacheWarmer] ✓ 预热成功: {url}")
                else:
                    failed_count += 1
                    logger.logger(f"[CacheWarmer] ✗ 预热失败(空内容): {url}")

            except Exception as e:
                failed_count += 1
                logger.logger(f"[CacheWarmer] ✗ 预热失败: {url} - {e}")

        logger.logger(f"[CacheWarmer] 预热完成: 成功{warmed_count}, 失败{failed_count}")
