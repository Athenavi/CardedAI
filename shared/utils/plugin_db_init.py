...
class PluginDatabaseInitializer:
    ...
    def discover_plugins_needing_db(self) -> List[Dict[str, Any]]:
        ...
        for item in self.plugins_dir.iterdir():
            ...
            try:
                ...
                logger.logger(f"f"[PluginDBInit] Failed to read metadata for {item.name}: {e}")
    ...
    def initialize_plugin_db(self, plugin_slug: str) -> bool:
        ...
        try:
            ...
            if not plugin_path.exists():
                logger.logger(f"f"[PluginDBInit] Plugin file not found: {plugin_slug}")
                return False
            ...
            if hasattr(module, init_func_name):
                ...
                init_func()
                logger.logger(f"f"[PluginDBInit] ✓ Initialized database for {plugin_slug}")
                return True
            else:
                logger.logger(f"f"[PluginDBInit] ⚠ No init function found for {plugin_slug}")
                return False

        except Exception as e:
            logger.logger(f"f"[PluginDBInit] ✗ Failed to initialize {plugin_slug}: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def initialize_all(self) -> Dict[str, bool]:
        ...
        logger.logger(f"f"[PluginDBInit] Found {len(plugins)} plugins requiring database initialization")
        logger("=" * 60)
        ...
        logger("\n" + "=" * 60)
        logger.logger(f"f"[PluginDBInit] Initialization complete:")
        logger.logger(f"f"  Total: {len(plugins)}")
        logger.logger(f"f"  Success: {success_count}")
        logger.logger(f"f"  Failed: {fail_count}")
    ...
    def reset_plugin_db(self, plugin_slug: str) -> bool:
        ...
        """
        try:
            logger.logger(f"f"[PluginDBInit] Resetting database for {plugin_slug}...")

            # 删除现有数据库
            if plugin_db.delete_plugin_db(plugin_slug):
                logger.logger(f"f"[PluginDBInit] Deleted existing database")

            # 重新初始化
            return self.initialize_plugin_db(plugin_slug)

        except Exception as e:
            logger.logger(f"f"[PluginDBInit] Failed to reset database: {e}")
            return False
    ...
    if args.list:
        ...
        if databases:
            ...
            for db in databases:
                ...
                logger.logger(f"f"  {db['plugin_slug']:30s} {size_kb:8.2f} KB")
        else:
