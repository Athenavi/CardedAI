...
export function saveTokens(accessToken: string, refreshToken?: string): void {
    ...
    document.cookie = `access_token=${accessToken}; expires=${expirationDate.toUTCString()}; path=/; SameSite=Strict; Secure`;

    if (refreshToken) {
        const refreshExpirationDate = new Date();
        refreshExpirationDate.setTime(refreshExpirationDate.getTime() + (7 * 24 * 60 * 60 * 1000));
        document.cookie = `refresh_token=${refreshToken}; expires=${refreshExpirationDate.toUTCString()}; path=/; SameSite=Strict; Secure`;
    }
}
