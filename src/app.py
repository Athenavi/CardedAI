...
def safe_run(func_name: str, func, *args, **kwargs):
    ...
    try:
        ...
        logger.logger(f"f"[{func_name}] 失败: {e}", exc_info=True)
        return None
    ...
    try:
        ...
        for module_path, prefix, tags, result in load_results:
            ...
            if error is not None:
                if req:
                    logger.logger(f"f"{worker_info} v2 必需模块加载失败: {module_path} - {error}")
                    raise error
                else:
                    failed_count += 1
        ...
        logger.logger(f"f"{worker_info} API v2 模块未找到: {e}")
        raise
...
def register_error_handlers(app: FastAPI):
    ...
    async def general_exception_handler(request: Request, exc: Exception):
        from src.unified_logger import default_logger as logger
        logger.logger(f"f"General error: {exc}")
