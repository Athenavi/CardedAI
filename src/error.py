...
def error(status_code, message):
    ...
    title = "页面未找到" if status_code == 404 else "发生错误"
    simple_error_html = f"""<!DOCTYPE html>
<html>
<head>
    <title>{title} - {status_code}</title>
    <meta charset="utf-8">
    <style>
        ...
        h2 {{color: #2c3e50;font-size: 24px;margin-top: 10px;}}
        p {{font-size: 18px;color: #666;margin: 20px 0;}}
        a {{color: #3498db;text-decoration: none;font-weight: bold;font-size: 16px;}}
        a:hover {{text-decoration: underline;}}
        .home-link {{display: inline-block;margin-top: 20px;padding: 10px 20px;background-color: #3498db;color: white;border-radius: 5px;text-decoration: none;}}
        .home-link:hover {{background-color: #2980b9;text-decoration: none;}}
    </style>
</head>
<body>
