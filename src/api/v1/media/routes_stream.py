...
async def get_media_file_by_id(
    ...
    try:
        ...
        if not media:
            logger.logger(f"f"[ERROR] 媒体文件不存在 - ID: {media_id}")
        ...
        # 处理 S3 存储
            ...
            logger.logger(f"f"  [ERROR] 不支持的存储类型: '{file_hash.storage_path}'")
            logger.logger(f"f"  文件是否存在于标准路径: {file_path.exists()}")
            logger.logger(f"
