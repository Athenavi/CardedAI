...
async def setting_profiles_back(user_id: int, user_info, cache_instance, avatar_url_api: str,
    ...
    try:
        ...
        logger.error(f"Error in setting_profiles_back: {e}")
...
async def confirm_email_back(
    ...
    await bind_email(user_id, new_email, db)
    # cache_instance.delete_memoized(current_app.view_functions['api.api_user_profile'], user_id=user_id)
...
async def update_avatar_api(
    ...
    try:
        # 调试日志
        logger.error(f"[Avatar Upload] === START ===")
        logger.error(f"[Avatar Upload] Received request from user: {current_user.id}")
        logger.error(f"[Avatar Upload] File name: {file.filename}")
        logger.error(f"[Avatar Upload] Content type: {file.content_type}")
        logger.error(f"[Avatar Upload] File size attr: {file.size if hasattr(file, 'size') else 'unknown'}")

        # 验证文件类型
        allowed_types = ['image/jpeg', 'image/png', 'image/webp', 'image/jpg']
        if file.content_type not in allowed_types:
            logger.error(f"[Avatar Upload] Invalid content type: {file.content_type}")
            return JSONResponse(
        ...
        logger.error(f"[Avatar Upload] Actual file size: {len(file_content)} bytes")
        ...
        logger.error(f"[Avatar Upload] Success! Avatar URL: {avatar_url}")
        logger.error(f"[Avatar Upload] === END ===")
