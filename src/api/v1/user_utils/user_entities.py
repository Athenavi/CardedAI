...
def check_user_conflict(zone: str, value: str, db: Session) -> bool:
    try:
        ...
        logger(f"Error getting user list: {e}")
        return False


async def check_user_conflict_async(zone: str, value: str, db: AsyncSession) -> bool:
    """异步版本的用户冲突检查"""
    try:
        if zone == 'username':
            user_query = select(User).where(User.username == value)
        ...
        logger(f"Error checking user conflict: {e}")
        return False


async def save_uploaded_avatar(file: UploadFile, user_id: int, db):
    try:
        # 生成唯一的文件名
        ...
        logger(f"Database error saving avatar: {e} by user {user_id}")
        if isinstance(db, AsyncSession):
            await db.rollback()
        else:
            db.rollback()
        ...
        logger(f"Error saving avatar file: {e} by user {user_id}")
        raise
...
async def db_save_bio(user_id: int, bio: str, db):
    try:
        ...
        logger(f"Error saving bio: {e} by user {user_id} bio: {bio}")
        if isinstance(db, AsyncSession):
            await db.rollback()
        else:
            db.rollback()
...
async def change_username(user_id: int, new_username: str, db):
    ...
    try:
        ...
        logger(f"Error changing username: {e} by user {user_id} new username: {new_username}")
        if isinstance(db, AsyncSession):
            await db.rollback()
        else:
            db.rollback()
...
async def bind_email(user_id: int, param: str, db) -> bool:
    conflict = await check_user_conflict_async('email', param, db)
    if conflict:
        return False
    try:
        if isinstance(db, AsyncSession):
        ...
        logger(f"Error binding email: {e} by user {user_id} email: {param}")
        if isinstance(db, AsyncSession):
            await db.rollback()
        else:
            db.rollback()
...
async def get_avatar(domain: str, user_identifier, identifier_type: str = 'id', db=None):
    ...
    try:
        ...
        logger(f"Error fetching avatar: {e}")
